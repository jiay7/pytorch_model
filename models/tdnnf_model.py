...
class SOrthConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, padding_mode='zeros'):
        ...
        kwargs = {'bias': False}
        # self.conv.weight.shape=(out_channels,in_channels,kernel_size)
...
class FTDNNLayer(nn.Module):
    ...
    def forward(self, x, semi_ortho_step=True):
        ...
        x = self.factor1(x.transpose(1, 2),semi_ortho_step=semi_ortho_step)
...
class FTDNNLayer_type2(nn.Module):

    def __init__(self, in_dim, out_dim, bottleneck_dim, context_size=2, dilations=None, paddings=None, alpha=0.0):
        '''
        2 stage factorised TDNN
        '''
        super(FTDNNLayer_type2, self).__init__()
        paddings = [1, 1] if not paddings else paddings
        dilations = [2, 2] if not dilations else dilations
        assert len(paddings) == 2
        assert len(dilations) == 2
        self.factor1 = SOrthConv(
            in_dim, bottleneck_dim, context_size, padding=paddings[0], dilation=dilations[0])
        self.factor2 = nn.Conv1d(bottleneck_dim, out_dim, context_size,
...
class TDNN(nn.Module):
    ...
    def __init__(
        ...
        # Conv1d 输入(batch, input_features,in_seq_len) 输出(batch, output_features,out_seq_len)
        ...
        #self.kernel.weight.shape = (self.output_dim,self.input_dim,self.stride)
    ...
    def forward(self, x):
        ...
        assert (d == self.input_dim), 'Input dimension was wrong. Expected ({}), got ({})'.format(
            self.input_dim, d)
        #print("kernel.weight.shape:",self.kernel.weight.shape)
        #print("x.shape:",x.transpose(1, 2).shape)
        x = self.kernel(x.transpose(1, 2))
        #print("kernel output.shape:",x.shape)
