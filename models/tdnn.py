...
#         self.check_valid_context(context) #context = [-3,3]

#         kernel_size = len(context) # 2
#         if len(context) == 1:
#             dilation = 1
#             padding = 0
#         else:
#             delta = [context[i] - context[i - 1] for i in range(1, len(context))] #[6]
#             dilation = delta[0] #6
#             padding = max(context) #3
...
class TDNN(torch.nn.Module):
    def __init__(self,
        ...
        context = sorted(context) #context = [0,3] or [-3,0]

        kernel_size = len(context) # 2
        if len(context) == 1:
            dilation = 1
        else:
            delta = [context[i] - context[i - 1] for i in range(1, len(context))] #[3]
            dilation = delta[0] #3
        self.temporal_conv = weight_norm(
            torch.nn.Conv1d(
                input_dim,
